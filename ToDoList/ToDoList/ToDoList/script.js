...
/*var url = '';
//função pro alert sumir sozinho
...
//--------------------------------------------------------------------------------------------
 // Função adicionar o elemento na lista na ordem de prioridade
function add(){
  let inser = false;
  let i = 0;
...
//--------------------------------------------------------------------------------------------
 // Função para retornar o elemento do inicio da lista sem remove-lo
 function eleInicio(){
  minhaLista.peek();
  const firstTask = minhaLista.peek();
  mostrarFirstTask(firstTask);
 }

 function mostrarFirstTask(firstTask) {
  const mensagem = document.getElementById("first-task");
  mensagem.innerHTML ="Tarefa no Início: "+ firstTask.descricao;
...
//Função para mostrar mensagem de remoção mostrando quanto tempo foi necessário para realizar uma tarefa
function mostrarMensagemRemocao(tarefaRealizada) {
  taskR = tarefaRealizada;
  let dataAtual = obterDataAtual();
  let horaAtual = obterHoraAtual();
  let difData = calcularDiferencaDias(taskR.data, dataAtual);
  let difHora = calcularDiferencaHoras(taskR.hora, horaAtual);
  const mensagem = document.getElementById("mensagem-remocao");
  mensagem.innerHTML ="Tarefa realizada: "+ tarefaRealizada.descricao + " - Tempo necessário para concluí-la: "+ difData + " dias e "+difHora+" horas.";
  mensagem.style.display = "block";
}
//--------------------------------------------------------------------------------------------
// Função para mostrar a tarefa que está a mais tempo na lista
function maisTempo(){
  if(!minhaLista.isEmpty()){
    let maisAntTarefa = minhaLista.head.dado;
  
    for(const tarefa of minhaLista){
      maisAntTarefa = comparaTarefasDataHora(maisAntTarefa, tarefa);
    }
    mostrarMaisTempo(maisAntTarefa);
  }else{
      mensagem.innerHTML = "Lista Vazia";
  }
}

function mostrarMaisTempo(oldTask) {
  const mensagem = document.getElementById("old-task");
  mensagem.innerHTML ="Tarefa a mais tempo na lista: "+ oldTask.descricao;
  mensagem.style.display = "block";
}
//--------------------------------------------------------------------------------------------
//Função para mostrar mensagem de remoção mostrando quanto tempo foi necessário para realizar uma tarefa
